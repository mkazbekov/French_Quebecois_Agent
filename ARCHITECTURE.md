...
One Next.js 16 app, one process, one URL. Voice is realtime WebRTC; long-term
learner memory lives in Letta. Nothing else stores learner state.

```
Browser (React)
  RealtimeSession (@openai/agents-realtime, WebRTC, mic + speaker)
      │  ephemeral key + tutor instructions
      ▼
Next.js API routes (server, holds the real API keys)
  POST /api/realtime/session   load learner state → build instructions → mint ek_ key
...
4. End (button, or transport drop) → `POST /api/session/end` with transcript +
   evidence → OpenAI structured review (delta) → deterministic merge into the
...
- `@openai/agents-realtime` over raw WebRTC: handles SDP, mic, playback,
  interruption, transcripts; the docs call it the recommended browser path.
