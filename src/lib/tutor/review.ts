...
- Errors: group by underlying pattern (e.g. "passé composé auxiliary: être vs avoir"), not by sentence. Reuse the wording of an existing registry pattern when it is the same issue so it can be deduplicated. occurrences = how many times it happened this session. "preferred" must be natural Québec/standard French. unit_id: the syllabus unit (from "Syllabus units in play" or any id you can see in the registry) that teaches the fix, "" if none fits.
